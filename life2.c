 ...
 * The game plays 100 rounds, printing to the screen each time.  '*' printed
...
void initialize_board (int board[][BOARD_HEIGHT]) {
	...
	// Fill all cells randomly with 1 or 0
	for (i=0; i<BOARD_WIDTH; i++) for (j=0; j<BOARD_HEIGHT; j++)
		board[i][j] = (rand() % 2 & 1) ?1 :0;
}
...
void print (int board[][BOARD_HEIGHT])
	...
	for (j=0; j<BOARD_HEIGHT; j++) {
		...
		printf("\033[%d;0H", j);   // Cursor to left edge and curr row
...
int main (int argc, char *argv[]) {
	...
	struct timespec TS;
	TS.tv_nsec = 25 * 10000000;	TS.tv_sec = 0;  // '25' = 1/4 sec
